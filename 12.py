...
#Createing a random dataset of 100 rows and 30 columns.
np.random.seed(42)  # For reproducibility
dataset = pd.DataFrame(np.random.randint(1, 201, size=(100, 30)), columns=[f'col{i}' for i in range(1, 31)])

#Replacing all the values with NA in the dataset defined between [10, 60].
dataset.loc[:, 'col10':'col60'] = np.nan

# Printing the count of number rows having missing values.
...
#  Replacing all the NA values with the average of the column value.
dataset.fillna(dataset.mean(), inplace=True)

#Finding the Pearson correlation among all the columns and plot a heatmap.
correlation_matrix = dataset.corr()
sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm')
plt.title("Pearson Correlation Heatmap")
plt.show()

# Selecting those columns having correlation <= 0.7.
...
#Replacing all the values in the dataset with 1 if value <= 0.5 else with 0.
